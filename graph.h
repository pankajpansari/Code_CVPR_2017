...
public:
	...
	// returns arc capacity of SOURCE->i minus arc capacity of i->SINK
	tcaptype get_t_arc_cap(node_id i);

	// returns arc capacity of arc a
	captype get_arc_cap(arc* a);
	...
	void set_t_arc_cap(node_id i, tcaptype t_arc_cap);
	void set_arc_cap(arc* a, captype arc_cap);
	...
	inline void Graph<captype,tcaptype,flowtype>::add_tweights(node_id i, tcaptype cap_source, tcaptype cap_sink)
{
	...
	nodes[i].t_arc_cap = cap_source - cap_sink;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename Graph<captype,tcaptype,flowtype>::arc* Graph<captype,tcaptype,flowtype>::get_rev_arc(arc* a) 
{
	return a->sister; 
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline tcaptype Graph<captype,tcaptype,flowtype>::get_t_arc_cap(node_id i)
{
	assert(i>=0 && i<node_num);
	return nodes[i].t_arc_cap;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline captype Graph<captype,tcaptype,flowtype>::get_arc_cap(arc* a)
{
	assert(a >= arcs && a < arc_last);
	return a->arc_cap;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::set_t_arc_cap(node_id i, tcaptype t_arc_cap)
{
	assert(i>=0 && i<node_num); 
	nodes[i].t_arc_cap = t_arc_cap;
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::set_arc_cap(arc* a, captype arc_cap)
{
	assert(a >= arcs && a < arc_last);
	a->arc_cap = arc_cap;
}
