 ...
 *
 * Usage:
 *  [L E] = hierPn_mex(sparseG, Dc, hop, init_labels)
 *
 * Inputs:
...
void hierPn(int nout, mxArray* pout[], int nin, const mxArray*pin[]);
template<class T>
    ...
    const mxArray* pin[])
{
    if ( nout == 0 )
        return;
    if ( nin != 4 && nin != 5 )
        mexErrMsgIdAndTxt("hierPn:inputs","Must have 4 or 5 inputs");
    
    if (mxIsComplex(pin[1]))
        mexErrMsgIdAndTxt("hierPn:inputs","Dc must be a real-valued matrix");
    ...
    switch ( mxGetClassID(pin[1]) ) {
        case mxSINGLE_CLASS:
            ...
            return hierPn<float>(nout, pout, nin, pin);
        case mxDOUBLE_CLASS:
            // mexPrintf("Constructing double precision energy\n");            
            return hierPn<double>(nout, pout, nin, pin);
            default:
                mexErrMsgIdAndTxt("hierPn:inputs","Dc must be a non-integer matrix");
    }
    return;
}
...
void hierPn(int nout, mxArray* pout[], int nin, const mxArray* pin[])
{
    ...
    if ( ! mxIsSparse(pin[0]) || ! mxIsDouble(pin[0]) || mxIsComplex(pin[0]))
        mexErrMsgIdAndTxt("hierPn:inputs","sparseG must be a sparse double matrix");
    // check pin[0] is square
    const mwSize *spd = mxGetDimensions(pin[0]);
    if (spd[0] != spd[1])
        mexErrMsgIdAndTxt("hierPn:inputs","sparseG must be a square matrix");
    ...
    if (sdc[1] != spd[0])
        mexErrMsgIdAndTxt("hierPn:inputs","Dc must have %d columns to match graph structure", spd[0]);
    nLabel = sdc[0];
    
    
    // check pin[2] is struct array with proper feilds
    if ( mxGetClassID(pin[2]) != mxSTRUCT_CLASS )
        mexErrMsgIdAndTxt("hierPn:inputs","hop must be a struct array");
    nHigher = mxGetNumberOfElements(pin[2]);
    // expecting HOP_N_OF_FIELDS fieds
    if ( mxGetNumberOfFields(pin[2]) != HOP_N_OF_FIELDS )
        mexErrMsgIdAndTxt("hierPn:inputs","hop must have %d fields", HOP_N_OF_FIELDS);
    ...
    for ( ii = 0; ii < HOP_N_OF_FIELDS ; ii++ ) {
        ...
        if ( hop_fields_indices[ii] < 0 )
            mexErrMsgIdAndTxt("hierPn:inputs","hop is missing %s field", HOP_FIELDS[ii]);
    }


    termType *Lambda = (termType*)mxGetData(pin[3]);
    ...
    for ( ii = 0 ; ii < nHigher; ii++ ) {
        ...
        if ( mxGetNumberOfElements(xw) != n ) {
            delete energy;
            delete[] ind;
            mexErrMsgIdAndTxt("hierPn:inputs","hop %d: number of indices is different than number of weights", ii);
        }
        ...
        if ( mxGetNumberOfElements(xgamma) != nLabel+1 ) {
            ...
            delete[] ind;
            delete[] w;
            mexErrMsgIdAndTxt("hierPn:inputs","hop %d: must have exactly %d gamma values", ii, nLabel+1);
        }
        ...
        if ( energy->SetOneHOP(n, ind, w, gamma, Q) < 0 ) {
            delete energy;
            delete[] gamma;
            mexErrMsgIdAndTxt("hierPn:inputs","failed to load hop #%d", ii);
        }
    ...
    if ( nin == 5 ) {
        ...
        if ( mxGetNumberOfElements(pin[4]) != nVar )
            mexErrMsgIdAndTxt("hierPn:inputs","Initial guess of labeling must have exactly %d elements", nVar);
        GetArr(pin[4], solution, -1); // convert Matlab's 1-ind labeling to C's 0-ind labeling
        //for (int k = 0;k<nVar; k++) cout<<solution[k]<<" ";
    } else {
        // default initial labeling
        //cout<<"\n\tDefault initialization: all set to zero";
...
void GetArr(const mxArray* x, T* arr, T bias)
{
    ...
    switch (mxGetClassID(x)) {
        ...
        case mxUINT64_CLASS:
            ...
            return;
        default:
            mexErrMsgIdAndTxt("hierPn:GetArr","unsupported data type");
    }
}
