...
private :
    ...
    termType expand(int label, termType E_old)
    {
	   ...
	   if(energy->cliqueWeights[i]>0 && energy->lambda>0){

            for( int ii = 0; ii<energy->higherElements[i];ii++) setA.insert(label_map[energy->higherIndex[i][ii]]);

            set<int> setAwithAlpha = setA; setAwithAlpha.insert(label);

            vector<int> vecA(setA.begin(), setA.end());
            vector<int> vecAwithAlpha(setAwithAlpha.begin(), setAwithAlpha.end());

            aux_count=0;
            aux_nodes = new node_id_t[setA.size()+1];
            ...
            for(int j = 0; j<vecA.size();j++) { // note that we can't directly access an element in set
                if(vecA[j] != label){

                    int currLabel = vecA[j];
                    ...
                    for (int k = 0; k<energy->higherElements[i];k++){
                        if(label_map[energy->higherIndex[i][k]]==vecA[j]){
