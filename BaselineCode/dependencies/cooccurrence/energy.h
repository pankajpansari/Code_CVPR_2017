...
class Energy
{
    private:
        ...
        termType *rhstMetricCost;
        termType *cliqueWeights; // weight corresponding to each clique
        termType lambda; // To be multiplied with the high order potentials
    ...
    public:
         ...
         */
		Energy(int nLabel, int nVar, int nPair, int nHigher, termType Lambda ):
            ...
            pairIndex(NULL), higherIndex(NULL), higherElements(NULL)
		{
            ...
            rhstMetricCost = new termType[nlabel * nlabel];
            cliqueWeights = new termType[nhigher];
        ...
        void SetRHSTMetricCost(const termType* dc)
        {
             memcpy(rhstMetricCost, dc, nlabel*nlabel*sizeof(termType));
        }

        void SetCliqueWeights(const termType* cw)
        {
             memcpy(cliqueWeights, cw, nhigher*sizeof(termType));
        }
            ...
            termType Q) // truncation term
        {
            ...
            if ( hoi == nhigher ){
                std::cout<<"\n\n\tCAUTION: no vacant slot for this HOpotential\n\n";
		...
		~Energy()
		{
            ...
            if(rhstMetricCost != NULL) delete[] rhstMetricCost;
            if(cliqueWeights != NULL) delete[] cliqueWeights;
