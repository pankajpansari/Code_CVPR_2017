...
public:
    ...
    termType minimize(int *solution, termType* ee = NULL)
    {
        ...
        //cout<<"outer_counter:"<<outer_counter<<"\n";
...
private :
    ...
    termType expand(int label, termType E_old)
    {
        ...
        bool alpha_present;
        node_id_t *aux_nodes;
        aux_nodes = new node_id_t[nlabel+1];


        // create auxiliary nodes shared by all the cliques
        for(i=0;i<=nlabel;i++) aux_nodes[i] = g->add_node();
        ...
        for(i = 0;i < nhigher; i++)
        {
            ...
            if(!alpha_present){
                ...
                if(k1alpha<0) { cout<<"\n\tCaution: k1alpha negative: "<<k1alpha<<"\n"; k1alpha = 0.0; }
                 ...
                 // add auxilary node z_{\alpha}
                g->add_tweights( aux_nodes[nlabel], 0.0, k1alpha ); // add unary
                for (int k = 0; k<energy->higherElements[i];k++){
                    g->add_edge(aux_nodes[nlabel], nodes[energy->higherIndex[i][k]], 0.0, k1alpha); // add pairwise
                }
            }
            ...
            for(int j = 0; j<clique_label_set[i].size();j++) {
                if(clique_label_set[i][j] != label){
                    ...
                    //aux_nodes[currLabel] = g->add_node(); // add auxilary node z_{l} where l is not equal to alpha
                    g->add_tweights( aux_nodes[currLabel], k2l, 0.0 ); // add unary

                    for (int k = 0; k<energy->higherElements[i];k++){
                        if(label_map[energy->higherIndex[i][k]]==clique_label_set[i][j]){
                            g->add_edge(aux_nodes[currLabel], nodes[energy->higherIndex[i][k]], k2l, 0.0); // add pairwise
                        }
                    }
    ...
    termType compute_energy(termType& ue, termType& pe, termType& he)
    {
        ...
        for(int c=0;c<nhigher;c++){
            for(i = 0; i<energy->higherElements[c];i++) {
                clique_label_set[c].push_back(label_map[energy->higherIndex[c][i]]);
            }
            sort( clique_label_set[c].begin(), clique_label_set[c].end() );
            it = unique( clique_label_set[c].begin(), clique_label_set[c].end() );
            clique_label_set[c].resize( distance(clique_label_set[c].begin(), it ) );
            ...
            he += clique_potential(&clique_label_set[c]);
        }
        he *= lambda;
        //cout<<he<<"\n";
